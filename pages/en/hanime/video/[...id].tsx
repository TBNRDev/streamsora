...
interface Video {
  name: string;
  streams: { url: string, size_mbs: number }[];
  cover_url: string;
  tags: { name: string }[];
  description: string;
  episodes: Episode[];
...
export async function getServerSideProps({ params }) {
  const { id } = params;
  const corsProxy = "https://m3u8-proxy-cors-lac-rho.vercel.app/cors?url=";
  ...
  try {
    ...
    const streamsWithCorsProxy = data.results[0].streams.map((stream: { url: string | number | boolean; }) => ({
      ...stream,
      url: `${corsProxy}${encodeURIComponent(stream.url)}`
    }));
