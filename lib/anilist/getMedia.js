...
export default function GetMedia(session, stats) {
  ...
  const fetchGraphQL = async (query, variables) => {
    const response = await fetch("https://scrive-worker.mext-exams.workers.dev/?u=https://graphql.anilist.co/", {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    ...
    // Rate limit function to limit API calls
    ...
    const fetchMediaData = rateLimit(
        async () => {
          const data = await fetchGraphQL(queryMedia, {
            username,
            status: status?.stats,
            sort: "ID_DESC",
          });
        ...
        1, // Limiting to 1 request at a time
